...
stringOfBits = get_binary_string(fileContent);
% disp(stringOfBits);
...
% disp(temperatureArray);
% disp("---------------------------------------------------");
depthArray = get_depths(dataPoints, probe, recorder);
% disp(depthArray);
dt = merge_arrays(temperatureArray,depthArray,dataPoints); % depth dt(:,1), temperature dt(:,2)

% Display values
% fprintf('%.2f', pi) 2 decimal places
% fprintf("%02i\n",a) 0 padding left to complete 2 digits
datetime =sprintf("%i-%i-%i %02i:%02i",month,day,year,hour,minute);
...
% Locate XBT deployment site on a map
lats = [latitude];
longs = [longitude];
% figure
% geoplot(lats,longs,'r*');
% geolimits([45 62],[-149 -123])
% geobasemap bluegreen
% figure
% plot(temperatureArray,-depthArray);


% function riderName = get_rider_name(stringOfBits,dataPoints,tempStartBit)
%     nameBitStart = tempStartBit + dataPoints*12; % end of temperature bits
%     numberOfRiderBlocks = get_value(stringOfBits,843,848,"number",1,0); % get number of bits assigned for rider name
%     riderName = get_value(stringOfBits,nameBitStart,nameBitStart+numberOfRiderBlocks*40,"ascii");
% end
